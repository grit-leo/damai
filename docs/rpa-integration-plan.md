...
模型调用放在采集服务侧，不放在浏览器插件里。插件完成规则测算后，可以调用：
...
请求体包含结构化 `result`、证据摘要和图片证据摘要。服务端读取环境变量：

```text
ZZZ_API_KEY=...
ZZZ_BASE_URL=https://api.zhizengzeng.com/v1
ZZZ_MODEL=chat-latest
```

服务端按 OpenAI-compatible `chat/completions` 方式请求智增增平台。若未配置密钥或模型调用失败，接口会返回本地规则兜底文案，保证比赛演示不断流。

约束：

- 插件前端不保存、不展示 API Key。
- 数值结论来自 `analyzer.analyze()` 的规则计算，模型只负责组织 Boss 复核意见。
- Prompt 明确要求不得编造价格、SKU、平台和结论。

## 9. 运费体系口径

利润测算已从固定单件运费升级为按重量体积计算的运费模板。公开物流口径通常先计算实重和体积重，体积重公式为 `长 × 宽 × 高 / 泡重系数`，再取较大值作为计费基础，并按首重/续重进位。

```text
体积重 = 长(cm) × 宽(cm) × 高(cm) × 件数 / 泡重系数
计费重 = max(实重 × 件数, 体积重)
结算计费重 = 首重 + ceil((计费重 - 首重) / 续重单位) × 续重单位
运费 = 首重运费 + 续重段数 × 续重运费 + 包材/附加费 + 偏远/冷链加收 - 运费补贴
贡利额 = 到手价 - 采购价 - 到手价 × 费用率 - 运费
建议采购价上限 = 到手价 × (1 - 费用率 - 目标贡利率) - 运费
```

当前 demo 默认参数为：实重 `1.1kg`，长宽高 `24 × 18 × 10cm`，泡重系数 `8000`，首重 `1kg / 3.2元`，续重 `0.5kg / 0.45元`。样例 SKU 体积重为 `0.54kg`，计费重按实重进位到 `1.5kg`，运费为 `3.65元`。不含运费时建议采购价为 `20.57元`，接入模板运费后建议采购价上限为 `16.92元`。这个差异是本项目相对现有站内定价助手的关键创新点之一。
